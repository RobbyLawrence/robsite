...
Having previously worked on the linear programming fomrulation for classical Sudoku formulation, I had a hunch for what variables to choose before starting work on the constraints. For this formulation, I'll consider a grid size of $9$ (although this formulation works for an arbitrarily sized grid!). Recall that we are searching for a feasible solution, rather than an optimal solution, so we won't consider an objective function for any of our variables.

We start off with a $9\times9\times9$ array of binary variables; we'll call it $x[9][9][9]$ - note that all the constraints below are $1$-indexed, but any respectable implentation will be $0$-indexed. For $i,j,k\in\{1,...,9\}$ a cell $(i,j)$, the binary variable will have the following value: $$
\begin{cases}
...
Now that we have variables, so what? These variables won't solve the grid themselves! We have to impose <u>constraints</u>. In any solution, we will have exactly one value per grid. That is,
$$
...
It's easier to think about problems like these by fixing a few variables. Fix $k, v\in \{1,...,9\}$. Then, exactly one cell should have both $x[i][j][k]=1$ and $a[i][j][v]=1$; that is, we constrain a single cell to have the value $k$ in the $i^{\text{th}}$ subgrid. Those of you with a little experience in logic might recognize this: a logical $\text{AND}$! Formalizing, we have the following constraint:
$$
...
Gurobi eased my worries. The presolve alone took care of the entire system, presoliving $348$ rows and $729$ columns in less than a hundredth of a second.
